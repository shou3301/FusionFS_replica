...
#include "params.h"
#include "util.h"
#include "./replication/replica_util.h"
...
int fusion_getattr(const char *path, struct stat *statbuf)
{
	...
	char path_prime[PATH_MAX];
	char path_replica[PATH_MAX];
	strcpy(path_prime, path);
	strcpy(path_replica, path);
	strcat(path_prime, ":prime");
	strcat(path_replica, ":replica");

	char res[PATH_MAX] = {0};
	// int status = zht_lookup(path, res);
	int status = zht_lookup(path_prime, res);

	//===========================================================
	// get replica addr list
	int numOfReps = getNumOfReplicas();
	char repAddrs[2 * PATH_MAX] = {0};
	int rep_lookup_status = zht_lookup(path_replica, repAddrs);
	char *addrs[5] = {0};
	char *pch;
	int j;
	// remember to add ; before the first addr
	if (rep_lookup_status != ZHT_LOOKUP_FAIL) {
		for (j = 0; j < numOfReps; ++j) {
			pch = strtok(repAddrs, ";");
			strcpy(addrs[j], pch);
		}
	}
	...
	if (ZHT_LOOKUP_FAIL == status) { /* if not found in ZHT */
		...
		if (access(fpath, F_OK)) { /*if it isn't on this node, copy it over*/
			...
			//=======================================================================
			if (ffs_recvfile_c("udt", res, "9000", fpath, fpath) != 0) {
				if (rep_lookup_status != ZHT_LOOKUP_FAIL) {
					int ffi;
					for (ffi = 0; ffi < numOfReps; ++ffi) {
						if (ffs_recvfile_c("udt", addrs[ffi], "9000", fpath, fpath) == 0) {
							log_msg("cshou debug ******************* transfer file from replica addr = %s", addrs[ffi]);
							break;
						}
					}
				}
...
int fusion_release(const char *path, struct fuse_file_info *fi)
{
	...
	zht_lookup(path_prime, nodeaddr);
	...
	if (rep_lookup_status == ZHT_LOOKUP_FAIL) {
		...
		for (j = 0; j < numOfReps; ++j) {
			pch = strtok(repAddrs, ";");
			strcpy(addrs[j], pch);
			log_msg("cshou debug ******************* getting replica info in relase with addr = %s", addrs[j]);
		}
	}
	//=============================================================

	// current node is the prime
	if (!strcmp(myip, nodeaddr)) {
		...
		if (iswritten) {
			...
			int ffi;
			for (ffi = 0; ffi < numOfReps; ++ffi) {
				ffs_sendfile_c("udt", addrs[ffi], "9000", fpath, fpath);
				log_msg("cshou debug ******************* updating replica on addr = %s", addrs[ffi]);
			}
			//===================================================================
		}
	...
	//===================================================================
	// check if current node is a replica or not
	char oldip[PATH_MAX] = {0};
	zht_lookup(path_prime, oldip);

	int isReplica = 0;
	int ri;
	for (ri = 0; ri < numOfReps; ++ri) {
		if (!strcmp(myip, addrs[ri])) {
			isReplica = 1;
			memset(addrs[ri], 0, strlen(addrs[ri]));
			strcpy(addrs[ri], oldip);
		}
	}
	//===================================================================
	...
	if (iswritten) { /*so it's a write mode*/
		...
		if (!isReplica) {
			...
			char newAddrs[2 * PATH_MAX] = {0};
			newAddrs[0] = ';';
			for (ffi = 0; ffi < numOfReps; ++ffi) {
				strcat(newAddrs, addrs[ffi]);
				if (ffi != numOfReps - 1)
					strcat(newAddrs, ";");
			}
			zht_update(path_replica, newAddrs);
		}
		//=============================================================
